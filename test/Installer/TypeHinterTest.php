...
class TypeHinterTest extends PHPUnit_Framework_TestCase
{
  ...
  {
    $array_param = $this->getMockBuilder('ReflectionParameter')->disableOriginalConstructor()->getMock();
    $array_param->expects($this->once())->method('isArray')->will($this->returnValue(true));

    $typed_param = $this->getMockBuilder('ReflectionParameter')->disableOriginalConstructor()->getMock();
    $typed_param->expects($this->once())->method('isArray')->will($this->returnValue(false));
    $typed_param->expects($this->any())->method('__toString')->will($this->returnValue('Parameter #0 [ <optional> DateTime $date ]'));

    $typed_required_param = $this->getMockBuilder('ReflectionParameter')->disableOriginalConstructor()->getMock();
    $typed_required_param->expects($this->once())->method('isArray')->will($this->returnValue(false));
    $typed_required_param->expects($this->any())->method('__toString')->will($this->returnValue('Parameter #0 [ <required> DateTime $date ]'));
